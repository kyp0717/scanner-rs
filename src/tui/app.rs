...
impl App {
    ...
    fn cmd_help(&mut self) {
        let help = [
            ...
            "  cycle                 Start cycling all 8 scanners",
    ...
    fn cmd_scan(&mut self, args: &[&str], rt: &tokio::runtime::Handle) {
        ...
        // Run scan (blocking in the current thread context)
        let mut results = tws::run_scan(&scanner_code, &host, &ports, 1, rows, min_price, max_price);

        if !results.is_empty() {
            // Enrich with Yahoo Finance
            self.push_output("Enriching with Yahoo Finance...");
            rt.block_on(async {
                enrich_results(&mut results).await;
            });
        }
    ...
    fn cmd_list(&mut self, args: &[&str]) {
        ...
        let xml = tws::fetch_scanner_params(&self.settings.host, &ports, 3);
        self.clear_output();
    ...
    fn run_poll_scanners(&mut self, rt: &tokio::runtime::Handle) {
        ...
        for &(code, cid) in ALERT_SCANNERS {
            ...
            if !results.is_empty() {
                rt.block_on(async { enrich_results(&mut results).await });
            }
    ...
    fn cmd_cycle(&mut self, args: &[&str], rt: &tokio::runtime::Handle) {
        if args.first().map(|s| s.to_lowercase()) == Some("stop".to_string()) {
            if !self.cycling {
                self.push_output("Not cycling");
                return;
            }
            self.cycling = false;
            self.push_output("Cycling stopped");
            return;
        }

        if args.is_empty() {
            if self.cycling {
                self.push_output(&format!("  Cycling: on  |  Cycles: {}", self.cycle_count));
            } else {
                self.cycling = true;
                self.cycle_count = 0;
                self.push_output("Cycling started -- all 8 scanners every 90s");
                self.alert_line = "Cycling active".to_string();
                // First cycle immediately
                self.run_cycle_scanners(rt);
            }
            return;
        }

        self.push_output("Usage: cycle [stop]");
    }

    fn run_cycle_scanners(&mut self, rt: &tokio::runtime::Handle) {
        // Similar to poll but displays consolidated table
        self.run_poll_scanners(rt);
        self.cycle_count += 1;

        // Display cycle results
        if let Some(ref db) = self.db {
            let today = rt.block_on(db.get_today()).unwrap_or_default();
            let now = chrono::Local::now().format("%H:%M:%S").to_string();
            self.push_output(&format!(
                "\n-- Cycle {} @ {} -- ({} stocks) --",
                self.cycle_count,
            ...
            if !today.is_empty() {
                self.push_output(&format!(
                    "{:<10}  {:<6}  {:>8}  {:>8}  {:>6}  {:>8}  {:>5}  {}",
                    "Time", "Symbol", "Last", "Chg%", "RVol", "Float", "Scans", "Catalyst"
                ));
                self.push_output(&"-".repeat(90));
                for s in &today {
                    ...
                    let flt = s.float_shares.map(|f| format!("{:.1}M", f / 1e6)).unwrap_or("-".into());
                    let scans = s.scanners.split(',').count();
                    let cat = s.catalyst.as_deref().unwrap_or("");
                    let cat = if cat.len() > 30 { &cat[..28] } else { cat };
                    self.push_output(&format!(
                        "{:<10}  {:<6}  {:>8}  {:>8}  {:>6}  {:>8}  {:>5}  {}",
                        time_str, s.symbol, price, chg, rvol, flt, format!("{scans}/8"), cat
                    ));
                }
            }
            self.alert_line = format!(
...
pub fn run_tui() -> Result<()> {
    ...
    loop {
        ...
        // Check poll timer
        if app.polling && poll_timer.elapsed() >= Duration::from_secs(60) {
            poll_timer = std::time::Instant::now();
            app.run_poll_scanners(&handle);
        }

        // Check cycle timer
        if app.cycling && cycle_timer.elapsed() >= Duration::from_secs(90) {
            cycle_timer = std::time::Instant::now();
            app.run_cycle_scanners(&handle);
        }
...
mod tests {
    ...
    #[test]
    fn test_cycle_stop_when_not_cycling() {
        let (mut app, rt) = app_with_rt();
        let handle = rt.handle().clone();
        app.handle_input("cycle stop", &handle);
        assert!(app.output_lines.iter().any(|l| l.contains("Not cycling")));
    }
}
